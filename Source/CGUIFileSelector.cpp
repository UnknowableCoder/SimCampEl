					 ...
					 Dragging(false), FileNameText(0), FileList(0), DialogType(type)
{
	...
	//Description
	irr::gui::IGUIStaticText * but1 = Environment->addStaticText(L"Favourites",irr::core::rect<s32>(20,10+yoffset,170,48+yoffset),false,false,this,-1);
...
bool CGUIFileSelector::checkExistingFile()
{
	...
	if (found>0)
	{
		...
		video::ITexture * flag = driver->getTexture("Icons/status-dialog-warning-icon64.png");
		Environment->addMessageBox(L"Warning",L"Should this file be overwritten?",
...
void CGUIFileSelector::populateWindowsFAV()
{
	...
	addPlacePaths(L"Desktop",(wchar_t *)strPath,driver->getTexture("Icons/places_desktop.png"));

	// Get the special folder path. (My documents)
	SHGetSpecialFolderPath(	0, strPath, CSIDL_PERSONAL, FALSE );
	addPlacePaths(L"My Documents",(wchar_t *)strPath,driver->getTexture("Icons/places_documents.png"));

	// Get the special folder path. (My pictures)
	SHGetSpecialFolderPath( 0, strPath, CSIDL_MYPICTURES, FALSE );
	addPlacePaths(L"My Pictures",(wchar_t *)strPath,driver->getTexture("Icons/places_pictures.png"));

	// Get the special folder path. (My music)
	SHGetSpecialFolderPath(	0, strPath, CSIDL_MYMUSIC, FALSE );
	addPlacePaths(L"My Music",(wchar_t *)strPath,driver->getTexture("Icons/places_folder.png"));

	// Get the special folder path. (My video)
	SHGetSpecialFolderPath( 0, strPath, CSIDL_MYVIDEO, FALSE );
	addPlacePaths(L"My Video",(wchar_t *)strPath,driver->getTexture("Icons/places_folder.png"));

	// Get the special folder path. (Public documents)
	SHGetSpecialFolderPath(	0, strPath, CSIDL_COMMON_DOCUMENTS, FALSE );
	addPlacePaths(L"Public Documents",(wchar_t *)strPath,driver->getTexture("Icons/places_folder.png"));

	// Get the special folder path. (Public pictures)
	SHGetSpecialFolderPath(	0, strPath, CSIDL_COMMON_PICTURES, FALSE );
	addPlacePaths(L"Public Pictures",(wchar_t *)strPath,driver->getTexture("Icons/places_folder.png"));

	// Get the special folder path. (Public music)
	SHGetSpecialFolderPath(	0, strPath, CSIDL_COMMON_MUSIC, FALSE );
	addPlacePaths(L"Public Music",(wchar_t *)strPath,driver->getTexture("Icons/places_folder.png"));

	// Get the special folder path. (Public videos)
	SHGetSpecialFolderPath( 0, strPath, CSIDL_COMMON_VIDEO, FALSE );
	addPlacePaths(L"Public Video",(wchar_t *)strPath,driver->getTexture("Icons/places_folder.png"));
#endif
}
...
void CGUIFileSelector::populateLinuxFAV()
{
	...
	//addPlacePaths(L"User Home folder",L"~/",driver->getTexture("Icons/places_folder.png"));

	addPlacePaths(L"Home",L"/home",driver->getTexture("Icons/places_folder.png"));

	addPlacePaths(L"Media",L"/media",driver->getTexture("Icons/places_folder.png"));

	addPlacePaths(L"Usr",L"/usr",driver->getTexture("Icons/places_folder.png"));

	addPlacePaths(L"Dev",L"/dev",driver->getTexture("Icons/places_folder.png"));

	addPlacePaths(L"Mnt",L"/mnt",driver->getTexture("Icons/places_folder.png"));
